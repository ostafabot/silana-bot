...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
  ...
  let res = await apk(text);
        ...
        if (res.size > 993000000) {
            m.react(error)
            throw '*ملف  وانسح بي تطبقات اقل من 100ميغا لكي لا يتوقف البوت بي سبب ضغط APK كبير جدًا. الحد الأقصى لحجم التنزيل هو 990 ميجابايت*.';
          }
  ...
  await conn.sendMessage(m.chat, {
    text: `*جاري تحميل* ${info.name}...\n\nسعيد انك تستعمل redix بوت وسأكون مسرورا 😄 إن انضممت لأنستغرامي\ninstagram.com/mostaf.fc`,
  });
